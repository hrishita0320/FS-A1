...
public int maxEvents(int[][] events){
    ...
    root = buildSegmentTreeNode(firstDay,lastDay);
    int count = 0;
...
private SegmentTreeNode buildSegmentTreeNode(int start,int end){
    if(start>end){
        return null;
    }
    ...
    if(start!=end){
        int mid = (start+end)/2;
        node.left = buildSegmentTreeNode(start, mid);
        node.right = buildSegmentTreeNode(mid+1, end);
    }
    return node;
}
...
private int query(SegmentTreeNode curr,int left,int right){
    ...
    if(mid>=right){
        ...
        return Math.min(query(curr, left, mid),query(curr.right, mid+1, right));
    }
}
