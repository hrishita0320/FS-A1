...
class ScreenPlayTwoPointer{
    public static void main(String[] args){
        ...
        System.out.println(findRefill(arr,trivikram,vamshi));
        
    }
    public static int findRefill(int[] arr,int trivikram,int vamshi){
        int n = arr.length;
        int left =0;
        int right = n-1;
        int tempTrivi=trivikram;
        int tempVamshi = vamshi;
        int refill=0;
        while(left<=right){
            if(left==right){
                if(tempTrivi>=tempVamshi){
                    if(tempTrivi<arr[left]){
                        refill++;
                    }
                }
                else{
                    if(tempVamshi<arr[right]){
